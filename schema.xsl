 ...
 * Author: Enrico Ballarin Dolfin
 * Email: enrico@sed.ethz.chi
 * (with annotations and fixes by Philipp kaestli@sed.ethz.ch)
 *
 * modified from SC3ML 0.6 to QuakeML 1.2 RT stylesheet converter
...
xmlns="http://quakeml.org/xmlns/bed-rt/1.2"
    ...
    <!-- kae note: if your seiscomp configuration does not care that a publicid 
         is a valid quakeml resource reference, it should be prefixed here to be one.
         please set ID_PREFIX to the urn prefix of your agency. (following quakeml 
         recommendation, derive from the inverted DNS name) -->
    <xsl:variable name="ID_PREFIX" select="'smi:org.orfeus-eu/sc3a/'"/>
        ...
        <q:quakeml>
            <xsl:for-each select="$scsRoot/scs:seiscomp/scs:EventParameters">
                <eventParameters>
                    ...
                    <!-- Move all origin/stationMagnitudes and origin/magnitudes
                         to this level -->
                    <xsl:for-each select="scs:origin">
                        <xsl:for-each select="scs:stationMagnitude">
                            <!-- EBD <xsl:call-template name="genericNode"/> -->
...
<!-- temporary solution: delete TakeOffAngle. TODO: rather convert to TakeOffAngle/value -->
...
<!-- EBD INSERTED END -->
    ...
    <xsl:template match="scs:arrival">
        <xsl:element name="{local-name()}">
            <xsl:attribute name="{$PID}">
                <xsl:value-of select="concat(../@publicID, '/pick', substring-after(scs:pickID, 'pick'), '/arrival')"/>
            </xsl:attribute>
            <!--comment/-->
            <xsl:apply-templates/>
        </xsl:element>
    ...
    <xsl:template match="scs:amplitude/scs:amplitude/scs:value">
       <value><xsl:value-of select=". * 0.000001"/></value>
    </xsl:template>
...
-->
    ...
    <xsl:template name="convertID">
        <xsl:param name="id"/>
        <xsl:choose>
            <xsl:when test="not(starts-with($id, 'smi:'))">
